...
# scraping from wikivoyage.org due to its permissions
wikivoyage = [
...
def format_docs(docs):
    return "\n\n".join([format_document(doc, include_metadata=False) for doc in docs])
...
async def handle_message(message: cl.Message):
    ...
    rag_chain = (
        ...
        }) | RunnableLambda(lambda x: f"Context:\n{x['context']}\n\nQuestion: {x['question']}") | streaming_llm
    )
    ...
    task = asyncio.create_task(rag_chain.ainvoke({"question": message.content}))
...
#     "context": retriever | RunnableLambda(format_docs),
#     "question": RunnablePassthrough()
# }) | RunnableLambda(lambda x: f"Context:\n{x['context']}\n\nQuestion: {x['question']}") | streamed_llm
# )

# qa_chain = RetrievalQA.from_chain_type(llm=streamed_llm, retriever=retriever)